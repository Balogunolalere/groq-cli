...
from groq_cli import get_commands, handle_error, execute_command, display_and_select_command, display_and_select_solution
...
def test_execute_command(mock_run):
    ...
    mock_run.assert_called_once_with(['bash', '-c', command], check=True, text=True, capture_output=True)
